...
namespace MuseumApp
{
    public static class StarsRatingLib
    {
        ...
        public static void SetupStars(Image[] stars, string attactionID )
        {
            var userRating = Database.GetUserAttractionRating(attactionID);
            if(userRating != null)
            {
            ...
            var rating = Database.GetAttractionTotalRating(attactionID);
